...
class BadgeService
{
    ...
    public function verifierEtAttribuer(int $userId, int $formationId): array
    {
        ...
        foreach ($formation->modules as $module) {
            ...
            if ($completes >= $total) {
                ...
                if ($b) $nouveauxBadges[] = $b;
                break; // Un seul badge module_complete par formation
            }
        }
        ...
        // ── Badge : assidu (5 contenus aujourd'hui) ──────────
    ...
    private function attribuer(int $userId, int $formationId, string $code): ?array
    {
        ...
        // Vérifier si déjà attribué pour cette formation
        $existe = BadgeUtilisateur::where('user_id', $userId)
